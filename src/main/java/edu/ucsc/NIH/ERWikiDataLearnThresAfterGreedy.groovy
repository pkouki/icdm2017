...
ConfigBundle config = cm.getBundle("wikidata")

TimeNow = new Date();
def defaultPath = System.getProperty("java.io.tmpdir")
String dbpath = "./wikidata";
String dir = 'data' + java.io.File.separator+'wikidata' + java.io.File.separator + 'newSplit' + java.io.File.separator;
...
println "Threshold Step = " + thresholdStep

//we learn the initial weights from a grid search
int posFirstNameWeight = 25;
int posLastNameWeight = 1;
int posMaidenNameWeight = 1;

double negFirstNameWeight = 5;
double negLastNameWeight = 5;
double negMaidenNameWeight = 1;

int ageWeight = 5;
int negAgeWeight=10;
int posInfoWeight = 1;
int negGenderWeight=100;
int negLivingWeight=100;
int negInfoWeight = 100;

int posRelWeight = 1;
int negRelWeight = 1;

double parentSimWeight=10, childSimWeight=10, siblingSimWeight=10, spouseSimWeight=10;
double parentTranWeight=10, childTranWeight=10, siblingTranWeight=10, spouseTranWeight=10, negSpouseTranWeight=1;

double priorWeight = 1;
double bijectionWeight = 25;
double transWeight = 25;
...
for(int fold=1;fold<=numFolds;fold++){
	...
	if(fold==2 || fold==3){
		...
		siblingTranWeight=1;
		spouseTranWeight=1;
		negSpouseTranWeight=1;
	}
	
	//create the model, define the predicates and the rules
	PSLModel m = createModel(this, data, posFirstNameWeight, posLastNameWeight, posMaidenNameWeight,
		negFirstNameWeight, negLastNameWeight, negMaidenNameWeight,
		ageWeight, negAgeWeight, negGenderWeight, negLivingWeight,
		parentSimWeight, childSimWeight, siblingSimWeight, spouseSimWeight,
		parentTranWeight, childTranWeight, siblingTranWeight, spouseTranWeight,
		negSpouseTranWeight,
		priorWeight, bijectionWeight, transWeight,
		persInfRun, firstDegSimRun, firstDegTranRun, 
		bijRun, transRun);
	...
	if(fold==1){
		...
		InserterUtils.loadDelimitedDataTruth(insert, dir +"firstNameJaroSims");
		
		insert = data.getInserter(middleNameJaroSim, evidencePartitionCommon)
		InserterUtils.loadDelimitedDataTruth(insert, dir+"middleNameJaroSims");
		
		insert = data.getInserter(lastNameJaroSim, evidencePartitionCommon)
		InserterUtils.loadDelimitedDataTruth(insert, dir+"lastNameJaroSims");
		...
		InserterUtils.loadDelimitedDataTruth(insert, dir+"firstNameLevSims");
		
		insert = data.getInserter(middleNameLevSim, evidencePartitionCommon)
		InserterUtils.loadDelimitedDataTruth(insert, dir+"middleNameLevSims");
		
		insert = data.getInserter(lastNameLevSim, evidencePartitionCommon)
		InserterUtils.loadDelimitedDataTruth(insert, dir+"lastNameLevSims");
		
		insert = data.getInserter(maxJaroLevFirstName, evidencePartitionCommon)
		InserterUtils.loadDelimitedDataTruth(insert,  dir+"maxJaroLevFirstName");
		
		//load first degree relationship similarities
		if(firstDegSimRun){
			println "About to load the 1st degree Relationship similarities"
			insert = data.getInserter(sistersSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"sistersSim");
			
			insert = data.getInserter(brothersSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"brothersSim");
			
			insert = data.getInserter(fatherSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"fatherSim");
			
			insert = data.getInserter(motherSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"motherSim");
			
			insert = data.getInserter(daughtersSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"daughtersSim");
			
			insert = data.getInserter(sonsSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"sonsSim");
			
			insert = data.getInserter(spouseSim, evidencePartitionCommon)
			InserterUtils.loadDelimitedDataTruth(insert, dir+"spouseSim");
		}
	...
	Database dbValid = data.getDatabase(targetPartitionValid,
		[person, samePersonBlock, belongsToFamily,belongsToParticipant, 
			...
			firstNameJaroSim, middleNameJaroSim, lastNameJaroSim, 
			firstNameLevSim, middleNameLevSim, lastNameLevSim,
			maxJaroLevFirstName,
	...
	println "About to load the toPredict predicate to the target partition"
	//put everything you want to predict to the targetPartition
	...
	ConfusionMatrix confMat = computeConfusionMatrix(persons1_persons2_predvalues, persons1_persons2_matchvalues, bestThreshold, resultsFile + ".fold" + fold, true);
	computeStatsOneFold(confMat, matches, bestResults, true);
	...
	//i.e. store it in a treemultimap structure where the elements are sorted in 
	//decreasing probability value 
	sortedProbabilityPersonsMatched = parsePredictions(persons1_persons2_predvalues, bestThreshold)
	...
	computeStatsOneFold(confMat1_1, matches, bestResults1_1,true);
...
PSLModel createModel(Object object, DataStore data, 
	...
	int ageWeight, int negAgeWeight, int negGenderWeight, int negLivingWeight,
	double parentSimWeight, double childSimWeight, double siblingSimWeight, double spouseSimWeight, 
	double parentTranWeight, double childTranWeight, double siblingTranWeight, double spouseTranWeight,
	double negSpouseTranWeight,
	double priorWeight, double bijectionWeight, double transWeight, 
	boolean persInfRun, 
	boolean firstDegSimRun, 
	boolean firstDegTranRun, 
	...
	m.add predicate: "middleNameJaroSim", types:[ArgumentType.UniqueID,ArgumentType.UniqueID]
	m.add predicate: "lastNameJaroSim", types:[ArgumentType.UniqueID,ArgumentType.UniqueID]
	
	//this predicate is to enforce the bijection rule
	m.add predicate: "minJaroLevFirstName", types:[ArgumentType.UniqueID,ArgumentType.UniqueID]
	...
	m.add predicate: "middleNameLevSim", types:[ArgumentType.UniqueID,ArgumentType.UniqueID]
	...
	m.add rule : ( person(A) & person(B) & samePersonBlock(A,B) & middleNameJaroSim(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
	...
	m.add rule : ( person(A) & person(B) & samePersonBlock(A,B) & ~middleNameJaroSim(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
	...
	m.add rule : ( person(A) & person(B) & samePersonBlock(A,B) & middleNameLevSim(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> samePerson(A,B),  squared: sqPotentials, weight : posMaidenNameWeight
	...
	m.add rule : ( person(A) & person(B) & samePersonBlock(A,B) & ~middleNameLevSim(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> ~samePerson(A,B),  squared: sqPotentials, weight : negMaidenNameWeight
	...
	//similar age
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & similarAge(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
	...
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & ~sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> ~samePerson(A,B),  squared: oneDirPotentials, weight : negGenderWeight
	
	
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & ~sameLivingStatus(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> ~samePerson(A,B),  squared: oneDirPotentials, weight : negLivingWeight
	...
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & motherSim(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> samePerson(A,B),  squared: sqPotentials, weight : parentSimWeight
	...
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & daughtersSim(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> samePerson(A,B),  squared: sqPotentials, weight : childSimWeight
	...
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & sistersSim(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> samePerson(A,B),  squared: sqPotentials, weight : siblingSimWeight
	...
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & spouseSim(A,B) & sameGender(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> samePerson(A,B),  squared: sqPotentials, weight : spouseSimWeight
	
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & ~spouseSim(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> ~samePerson(A,B),  squared: sqPotentials, weight : negSpouseTranWeight
	
	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & ~motherSim(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> ~samePerson(A,B),  squared: sqPotentials, weight : negSpouseTranWeight

	m.add rule : ( person(A) & person(B)  & samePersonBlock(A,B) & ~fatherSim(A,B)
				&  belongsToFamily(A,W) & belongsToFamily(B,W)
				& belongsToParticipant(A,M) & belongsToParticipant(B,N)
				) >> ~samePerson(A,B),  squared: sqPotentials, weight : negSpouseTranWeight
	...
	m.add rule : ( person(P1) & person(P2) & person(S1) & person(S2) & sameGender(S1,S2)
				...
				) >> samePerson(S1,S2),  squared: oneDirPotentials, weight : parentTranWeight		
	...
	m.add rule : ( person(P1) & person(P2) & person(S1) & person(S2) & sameGender(S1,S2)
				...
				& belongsToParticipant(P1,M) & belongsToParticipant(S1,M)
				& belongsToParticipant(P2,N) & belongsToParticipant(S2,N) //A and C should belong to different tree
				) >> samePerson(S1,S2),  squared: sqPotentials, weight : spouseTranWeight
	...
	m.add rule : ( person(P1) & person(P2) & person(S1) & person(S2) & sameGender(S1,S2)
		...
		& belongsToParticipant(P1,M) & belongsToParticipant(S1,M)
		& belongsToParticipant(P2,N) & belongsToParticipant(S2,N) //A and C should belong to different tree
		) >> ~samePerson(S1,S2),  squared: sqPotentials, weight : negSpouseTranWeight
		...
		m.add rule : ( person(P1) & person(P2) & person(S1) & person(S2) & sameGender(S1,S2)
					...
					& belongsToParticipant(P1,M) & belongsToParticipant(S1,M)
					& belongsToParticipant(P2,N) & belongsToParticipant(S2,N) //A and C should belong to different tree
					) >> samePerson(S1,S2),  squared: oneDirPotentials, weight : siblingTranWeight
	...
	m.add rule : ( person(P1) & person(P2) & person(S1) & person(S2) & sameGender(S1,S2)
				...
				& belongsToParticipant(P1,M) & belongsToParticipant(S1,M)
				& belongsToParticipant(P2,N) & belongsToParticipant(S2,N) //A and C should belong to different tree
				) >> samePerson(S1,S2),  squared: oneDirPotentials, weight : childTranWeight
...
double findThreshold(HashMap<String, Double> persons1_persons2_matchvalues, 
	...
	EvalResultsSingle bestResults = new EvalResultsSingle();
	ConfusionMatrix confMat = new ConfusionMatrix();
	for(double threshold=0.01; threshold<1.0; threshold+=thresholdStep){
		println "Check for threshold value = " + threshold
		//Compute precision/recall/f-measure
		confMat = computeConfusionMatrix(persons1_persons2_predvalues, persons1_persons2_matchvalues, threshold, null, false);
		//println "Done with computing the confusion matrix -- now compute the stats for fold " + fold
		//compute stats call the function
		boolean update = computeStatsOneFold(confMat, matches, bestResults, true);
		if(update){
			bestThreshold = threshold;
			println "New threshold set at = " + bestThreshold;
		}
	}
	println "The best value for the threshold is " +  bestThreshold + " with stats: "
	println " Precision = " + bestResults.precision + " Recall = " + bestResults.recall + " Fmeasure = " + bestResults.fmeasure;
...
ConfusionMatrix computeConfusionMatrix(HashMap<String, Double> persons1_persons2_predvalues,
		HashMap<String, Double> persons1_persons2_matchvalues,
		double threshold, String fileName, boolean writeToResultsFile){
		
		ConfusionMatrix confMat = new ConfusionMatrix();
		
		//file where we will store the results
		def resultsFile;
		if(writeToResultsFile){
			if(fileName!=null){
	...
	boolean computeStatsOneFold(ConfusionMatrix confMat, Matches matches, 
							 EvalResultsSingle bestResults, boolean print){
							 				 
		if((confMat.classYes_predictedYes + confMat.classNo_predictedYes)==0 ||
			(confMat.classNo_predictedNo + confMat.classYes_predictedNo)==0 || 
			(confMat.classYes_predictedYes + confMat.classYes_predictedNo)==0||
			(confMat.classNo_predictedYes + confMat.classNo_predictedNo)==0)
			return false;
